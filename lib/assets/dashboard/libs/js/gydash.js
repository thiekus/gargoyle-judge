...
function getBaseUrl() {
    return window.location.origin + "/codeinfest";
}
...
function tickTock() {
    ...
    if (ttRemainTime > 0) {
        ...
        window.location = getBaseUrl() + "/dashboard?timeOut=yes";
    }
}
...
function progressiveDashboardPageLoadSuccess(resp, status, xhr, pushPage) {
    ...
    if (location.includes(getBaseUrl() + "/login")) {
...
function redefineAnchorVisit() {
    $("a").each(function () {
        ...
        var isDashboardLink = href.includes(getBaseUrl() + "/dashboard");
    ...
    $("form").each(function () {
        ...
        var isDashboardLink = action.includes(getBaseUrl() + "/dashboard");
...
function buildNotificationHtml(userName, avatar, description, link, since) {
    var html = "<a href=\"" + getBaseUrl() + link + "\" class=\"list-group-item list-group-item-action\">";
    html += "<div class=\"notification-info\">";
    html += "<div class=\"notification-list-user-img\"><img src=\"" + getBaseUrl() + avatar + "\" alt=\"" + userName + "Avatar\" class=\"user-avatar-md rounded-circle\"></div>";
...
function refreshAjaxNotifications() {
    $.ajax({
        type: "GET",
        url: getBaseUrl() + "/ajax/getNotifications",
        success: function (resp, status, xhr) {
